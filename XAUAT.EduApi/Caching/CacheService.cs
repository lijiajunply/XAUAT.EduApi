...
public class CacheService : ICacheService, IDisposable
{
        ...
        IMonitoringService? monitoringService = null)
    {
        ...
        _logger.LogInformation("CacheService initialized with strategy: {StrategyType}, LocalCacheMaxSize: {LocalCacheMaxSize}",
    ...
    public async Task<T> GetOrCreateAsync<T>(string key, Func<Task<T>> factory, TimeSpan? expiration = null, 
        CacheLevel level = CacheLevel.MultiLevel, int businessPriority = 5, CancellationToken cancellationToken = default)
    {
        var stopwatch = System.Diagnostics.Stopwatch.StartNew();
        ...
        try
        {
            ...
            switch (level)
            {
                ...
                case CacheLevel.Distributed:
                    ...
                    if (_redisCache != null && _redisCache.IsAvailable)
                    {
                        success = await _redisCache.SetAsync(key, value, actualExpiration, businessPriority, cancellationToken);
                        if (success)
                        {
                            _logger.LogTrace("Set cache (Redis): {Key}, Expiration: {Expiration}", key, actualExpiration);
                        }
                    }
                    break;
    ...
    /// <inheritdoc />
    public async Task<bool> RemoveAsync(string key, CacheLevel level = CacheLevel.MultiLevel, CancellationToken cancellationToken = default)
    {
        var stopwatch = System.Diagnostics.Stopwatch.StartNew();
        var success = true;
    ...
    /// <inheritdoc />
    public async Task<bool> ExistsAsync(string key, CacheLevel level = CacheLevel.MultiLevel, CancellationToken cancellationToken = default)
    {
        try
        {
            // 根据缓存级别检查缓存
            switch (level)
            {
                ...
                case CacheLevel.Distributed:
                    return _redisCache != null && _redisCache.IsAvailable && await _redisCache.ExistsAsync(key, cancellationToken);
                    
                case CacheLevel.MultiLevel:
                    // 先检查本地缓存，再检查分布式缓存
                    return _localCache.Exists(key) || 
                           (_redisCache != null && _redisCache.IsAvailable && await _redisCache.ExistsAsync(key, cancellationToken));
    ...
    /// <inheritdoc />
    public async Task<bool> ClearAsync(CacheLevel level = CacheLevel.MultiLevel, CancellationToken cancellationToken = default)
    {
        var stopwatch = System.Diagnostics.Stopwatch.StartNew();
        var success = true;
        ...
        CacheLevel level = CacheLevel.MultiLevel, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (remainingKeys.Any() && _redisCache != null && _redisCache.IsAvailable)
            {
        ...
        try
        {
            ...
            switch (level)
            {
                ...
                case CacheLevel.Distributed:
                    ...
                    if (_redisCache != null && _redisCache.IsAvailable)
                    {
                        success = await _redisCache.SetManyAsync(items, expiration, businessPriority, cancellationToken);
                    }
                    break;
    ...
    public async Task<CacheStatistics> GetStatisticsAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            {
                TotalItems = localStats.TotalItems + (redisStats?.IsAvailable == true ? await GetRedisItemCountAsync(cancellationToken) : 0),
                ...
                ExpiredItems = localStats.ExpiredItems + (redisStats?.IsAvailable == true ? await GetRedisExpiredCountAsync(cancellationToken) : 0),
                LevelStatistics = new Dictionary<CacheLevel, long>
                {
                    { CacheLevel.Local, localStats.TotalItems },
                    { CacheLevel.Distributed, redisStats?.IsAvailable == true ? await GetRedisItemCountAsync(cancellationToken) : 0 },
                    { CacheLevel.MultiLevel, localStats.TotalItems + (redisStats?.IsAvailable == true ? await GetRedisItemCountAsync(cancellationToken) : 0) }
                }
            };
        ...
        CacheLevel level = CacheLevel.MultiLevel, [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        if ((level == CacheLevel.Distributed || level == CacheLevel.MultiLevel) && 
            _redisCache != null && _redisCache.IsAvailable)
        {
    ...
    private async Task<bool> ProcessWarmupItemAsync(CacheWarmupItem warmupItem)
    {
        try
        {
            ...
            if (value == null)
            {
                _logger.LogWarning("Warmup task returned null value: {Key}", warmupItem.Key);
                return false;
            }
    ...
    /// <inheritdoc />
    public async Task<bool> SetStrategyAsync(string key, CacheStrategyType strategyType, 
    ...
    /// 设置本地缓存（异步包装）
    /// </summary>
    private async Task SetLocalCacheAsync<T>(string key, T? value, TimeSpan? expiration, int businessPriority, CancellationToken cancellationToken)
    {
    ...
    /// 获取Redis过期项数量（模拟实现）
    /// </summary>
    private Task<long> GetRedisExpiredCountAsync(CancellationToken cancellationToken)
    {
